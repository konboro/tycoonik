...
// IMPLEMENTACJA BRAKUJĄCYCH RENDERÓW (RANKING, GIEŁDA, OSIA, STATYSTYKI, ZNAJOMI)
    ...
    data.slice(0, 20).forEach((p, i) => {
        ...
        el.innerHTML = `
            <div class="flex gap-3">
                <div class="font-mono font-bold text-gray-500 w-6 text-right">#${i + 1}</div>
                <div class="font-bold text-white uppercase text-sm">${p.name}</div>
            </div>
            <div class="font-mono text-[#eab308] text-sm">${fmt(p.assetValue)} VC</div>
        `;
        list.appendChild(el);
    });
...
export function renderMarket(container) {
    if(!state.marketListings || state.marketListings.length === 0) {
        renderEmptyState(container, "BRAK OFERT RYNKOWYCH");
        return;
    }
    ...
    state.marketListings.forEach((l, i) => {
        ...
        el.innerHTML = `
            <div class="w-12 h-12 bg-black border border-[#333] flex items-center justify-center text-2xl text-gray-500">
                ${getIconHtml(l.vehicle.type)}
            </div>
            <div class="flex-grow">
                <div class="font-header font-bold text-white uppercase text-sm">${l.vehicle.customName || l.vehicle.title}</div>
                <div class="text-[10px] font-mono text-gray-500 uppercase">Sprzedawca: ${l.seller}</div>
            </div>
            <div class="text-right">
                <div class="text-[#eab308] font-mono font-bold text-lg">${fmt(l.price)} VC</div>
                <button class="btn-action text-[10px] px-2 py-1 mt-1" data-buy-market="${i}">KUP TERAZ</button>
            </div>
        `;
        container.appendChild(el);
    });
...
export function renderCharts(container) { // Mapowanie dla stats
    container.innerHTML = `<div class="h-full flex flex-col p-2">
        <div class="bg-[#1a1a1a] border border-[#333] p-2 mb-4 flex-grow">
            <h4 class="text-[10px] text-gray-500 font-bold uppercase mb-2">Przychody (24h)</h4>
            <div class="h-full w-full relative"><canvas id="earningsChart"></canvas></div>
        </div>
    </div>`;
    ...
    setTimeout(() => {
        ...
        new Chart(ctx, { 
            ...
            data: { 
                ...
                datasets: [{ 
                    label: 'VC', 
                    data: state.profile.earnings_history, 
                    borderColor: '#eab308', 
...
export function renderEnergyPrices(container) {
    ...
    if (tbody) {
        for (const c in state.economy.energyPrices) {
            for (const t in state.economy.energyPrices[c]) {
                const row = tbody.insertRow();
                row.className="border-b border-[#222] hover:bg-[#1a1a1a]";
                row.innerHTML=`<td class="px-4 py-2 font-bold text-white">${c}</td><td class="px-4 py-2">${t}</td><td class="px-4 py-2 text-right text-[#eab308]">${state.economy.energyPrices[c][t].toFixed(2)}</td>`;
            }
        }
    }
}
...
export function renderTransactionHistory(container) {
    ...
    history.forEach(t => {
        ...
        el.innerHTML = `
            <div>
                <div class="font-bold text-gray-300 text-xs uppercase">${t.description}</div>
                <div class="text-[10px] text-gray-600 font-mono">${new Date(t.timestamp).toLocaleTimeString()}</div>
            </div>
            <div class="font-mono font-bold ${t.amount >= 0 ? 'text-green-500' : 'text-red-500'}">${t.amount > 0 ? '+' : ''}${fmt(t.amount)} VC</div>
        `;
        container.appendChild(el);
    });
}
    ...
    container.innerHTML = `
        <div class="p-4 space-y-6">
            <div class="bg-[#1a1a1a] border border-[#333] p-4">
                <h3 class="text-xs font-bold text-gray-500 uppercase mb-2">Nazwa Korporacji</h3>
                <div class="flex gap-2">
                    <input type="text" id="company-name-input" value="${state.profile.companyName}" class="flex-grow bg-black border border-[#333] text-white p-2 font-mono text-sm focus:border-[#eab308] outline-none">
                    <button id="save-company-btn" class="btn-action px-4 text-sm">ZAPISZ</button>
                </div>
            </div>
            <div class="bg-[#1a1a1a] border border-[#333] p-4">
                <h3 class="text-xs font-bold text-gray-500 uppercase mb-2">Logotyp</h3>
                <div class="grid grid-cols-5 gap-2">
                    ${logos.map(l => `<button class="aspect-square bg-black border border-[#333] hover:border-[#eab308] flex items-center justify-center text-2xl transition" data-logo="${l}">${l}</button>`).join('')}
                </div>
            </div>
        </div>`;
}
...
export function renderFriendsTab(container) {
    ...
    container.innerHTML = `
        <div class="p-4 space-y-6">
            <div>
                <h3 class="text-xs font-bold text-gray-500 uppercase mb-2">Szukaj Partnera</h3>
                <div class="flex gap-2">
                    <input type="text" id="friend-search-input" class="flex-grow bg-black border border-[#333] text-white p-2 text-sm font-mono focus:border-[#eab308] outline-none" placeholder="NAZWA / EMAIL...">
                    <button id="search-friends-btn" class="bg-[#222] hover:bg-[#eab308] hover:text-black text-white px-3 border border-[#333] transition"><i class="ri-search-line"></i></button>
                </div>
                <div id="search-results" class="mt-2 space-y-1">
                     ${(searchResults||[]).map(r => `
                        <div class="bg-[#151515] p-2 border border-[#333] flex justify-between items-center">
                            <span class="text-white text-xs font-bold">${r.company_name}</span>
                            <button class="text-green-500 hover:text-white text-[10px] border border-green-900 bg-green-900/20 px-2 py-1 uppercase" data-add-friend="${r.id}">Dodaj</button>
                        </div>
                    `).join('')}
                </div>
            </div>
            ...
            <div class="border-t border-[#333] pt-4">
                <h3 class="text-xs font-bold text-gray-500 uppercase mb-2">Kontrahenci (${(friends||[]).length})</h3>
                <div class="space-y-2">
                    ${(friends||[]).map(f => `
                        <div class="bg-[#1a1a1a] p-3 border border-[#333] flex justify-between items-center group hover:border-[#eab308] cursor-pointer transition" data-open-chat="${f.id}">
                            <div>
                                <div class="font-bold text-white text-sm uppercase">${f.name}</div>
                                <div class="text-[10px] text-gray-500 font-mono">LVL ${f.level}</div>
                            </div>
                            <i class="ri-message-3-line text-gray-600 group-hover:text-[#eab308]"></i>
                        </div>
                    `).join('')}
                </div>
            </div>
...
export function renderAchievements(container) {
    ...
    for(const k in achievementsList) {
        ...
        const actionBtn = (isUnlocked && !isClaimed) 
            ? `<button class="btn-action text-[10px] px-2 py-1" data-claim="${k}">ODBIERZ</button>` 
            : (isClaimed ? '<span class="text-green-500 text-xs font-bold"><i class="ri-check-line"></i></span>' : '<i class="ri-lock-line text-gray-600"></i>');

        el.innerHTML = `
            <div class="text-2xl">${a.icon || '🏆'}</div>
            <div class="flex-grow">
                <div class="font-bold text-white text-xs uppercase">${a.title}</div>
