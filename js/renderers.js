...
export function renderInfrastructure(container) {
    ...
    for (const id in config.infrastructure) {
        ...
        const el = document.createElement('div'); 
        el.className = `bg-[#1a1a1a] border-l-2 border-[#eab308] p-3 mb-2 flex flex-col gap-2 hover:bg-[#222] transition border-b border-r border-t border-[#333]`;
        ...
        el.innerHTML = `
            <div class="flex items-center gap-3 cursor-pointer" onclick="toggleStationDetails('${id}')">
                ...
                <button class="text-gray-500 hover:text-white"><i class="ri-arrow-down-s-line"></i></button>
            </div>
        `;
        container.appendChild(el);
        
        // Kontener na szczegóły (rozkład jazdy)
        if (id === state.selectedStationId) { 
            const det = document.createElement('div'); 
            det.className = 'mt-2 p-2 bg-black border border-[#333] border-t-0 mb-2 text-xs font-mono animate-fade-in'; 
...
// Helper do przełączania (musi być globalny lub obsłużony w ui.js, tutaj robimy inline onclick hack dla uproszczenia renderera)
window.toggleStationDetails = (id) => {
    state.selectedStationId = state.selectedStationId === id ? null : id;
    // Wymuś przerysowanie tylko tego panelu, nie całej aplikacji
    const container = document.getElementById('mainList');
    if (container) {
        import('./ui-core.js').then(m => m.render());
    }
};
...
// 3. SZCZEGÓŁY STACJI (ROZKŁAD JAZDY - PRZYWRÓCONY)
export function renderStationDetails(id, container) {
    ...
    // LOGIKA DLA POCIĄGÓW (Digitraffic)
...
// ... (Pozostałe funkcje: renderGuildTab, renderVehicleCard, renderLootboxTab, renderRankings, renderMarket, renderCharts, renderStats, renderEnergyPrices, renderTransactionHistory, renderCompanyTab, renderFriendsTab, renderAchievements - BEZ ZMIAN, ale muszą być w pliku!) ...
...
export function renderStats(container) {
    ...
    const stats = calculatePeriodStats(timeframe); // Zakładamy, że helper jest dostępny lub go tu wkleję
